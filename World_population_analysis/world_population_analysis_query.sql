...
select name, median_age, fertility_rate from countries
where fertility_rate = (select max(fertility_rate) from countries)
;


-- Filtering the over populated to least populated country
select name, population, (case
when population >= 15000000 then 'Over Populated'
when population <= 10000000 then 'Slighly populated'
else 'least populated' end) as status from countries
order by population desc
;

-- Coutries that has the highest to lowest are of sq meter

select name, area_sq_km from countries
...
select * from countries;
select name, percent_one_year_change as fastest_growing_population from countries
order by fastest_growing_population desc
